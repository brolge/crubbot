# Crub - Ultimate Discord Bot

**Crub** is a powerful, feature-rich Discord bot designed to enhance your server experience with comprehensive moderation tools, engaging economy systems, utility features, and much more. Built with modern Discord.js v14 and PostgreSQL for optimal performance and data persistence.
...
Crub offers a complete suite of tools for Discord server management and community engagement:
...
[**Crub Setup Tutorial**](https://www.youtube.com/@TouchDisc)

## Docker Deployment (Recommended)

Crub is fully containerized for easy deployment.

1. **Clone the repository:**
   ```bash
   git clone https://github.com/codebymitch/Crub.git
   cd Crub
   ```
...
```bash
docker pull ghcr.io/codebymitch/crub:main
```
...
3. **Configure Environment Variables**
   ...
   POSTGRES_URL=postgresql://postgres:yourpassword@localhost:5432/crub
   ...
   - If your chosen `PORT` is already used, Crub automatically tries the next port(s)
...
Most users run Crub on a **single server** with `GUILD_ID` set (default tutorial setup). If you want commands to work in **every server** the bot is invited to, opt in with:
...
4. **Setup PostgreSQL Database** (Optional but recommended)
   ...
   createdb crub
   createuser crub
   psql -c "ALTER USER crub PASSWORD 'yourpassword';"
   psql -c "GRANT ALL PRIVILEGES ON DATABASE crub TO crub;"
   ```
...
Crub requires the following Discord intents:
- **Guilds**
- **Guild Messages**
- **Message Content**
- **Guild Members**
...
Crub is released under the MIT License. See [LICENSE](LICENSE) for details.

## Thank You

Thank you for choosing Crub for your Discord server! We're constantly working to improve and add new features based on community feedback.
